...
module pipelined_alu(
    ...
    reg [7:0]  regfile   [0:7];     // 8 general-purpose registers
    reg [15:0] acc;                 // FIX 1: widened to 16 bits to avoid MAC truncation
    ...
    // FIX 2: opcode 4'b1111 is treated as NOP - no writes, no side effects
    always @(posedge clk) begin
        if (flush) begin
            // Branch taken - flush ID/EX with NOP
            ...
            ID_EX_rs2      <= rs2;
            ID_EX_opcode   <= opcode;
            // regWrite: true for ALU ops (0-7), LD (8), MAC (10). NOT for ST (9), BEQ (12), NOP (15)
    ...
    // Forwarding for ST source (encoded in rd field)
    ...
    always @(*) begin
        ...
        alu_P_comb = ~(^alu_out_comb);    // P=1 means even number of 1s
    end
    ...
    // WB Stage
    //-------------------------
    // FIX 3: Gate `result` on MEM_WB_regWrite - holds last meaningful value
    // instead of being overwritten every cycle by NOP/ST pass-through
    always @(posedge clk) begin
        if (MEM_WB_regWrite) begin
            regfile[MEM_WB_rd] <= MEM_WB_data;
            result             <= MEM_WB_data;   // only updated on real register writes
        end
    end
